...
def payment_success(request):
    ...
    order.photo.save()
    return HttpResponseRedirect(redirect_to=f'https://localhost:8080/payment/success/{order.id}')
...
def payment_failed(request):
    transaction_id = request.POST.get('tran_id')
    order = get_object_or_404(Order, transaction_id=transaction_id)
    return HttpResponseRedirect(redirect_to=f'https://localhost:8080/payment/fail/{order.id}')
...
def payment_canceled(request):
    transaction_id = request.POST.get('tran_id')
    order = get_object_or_404(Order, transaction_id=transaction_id)
    return HttpResponseRedirect(redirect_to=f'https://localhost:8080/payment/cancel/{order.id}')
